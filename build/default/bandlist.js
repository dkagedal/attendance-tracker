...
class BandEvent extends LitElement {
  ...
  render() {
    return html`<div class='event'>
      ...
      ${this.participants.length ? html`<ul>${repeat(this.participants, p => p.id, (p, index) => html`<li>${p.id} ${p.data().attending}</li>`)}</ul>` : html``}
      </div>
      </div>`;
  }
  ...
  render() {
    ...
    return html`<div class='schedule'>
      ...
      <div class="eventlist ${this.visible ? "" : "hidden"}">
          ${repeat(this.events, e => e.id, (e, index) => html`<band-event eventref="${this.bandref}/events/${e.id}"
              type="${e.data().type}"
...
class BandList extends LitElement {
  ...
  constructor() {
    ...
    firebase.auth().onAuthStateChanged(user => {
      if (user) {
        let query = db.collection("bands").where("acl", "array-contains", user.uid);
